...
public interface ReportExecutionHistoryRepository extends JpaRepository<ReportExecutionHistory, Long>, JpaSpecificationExecutor<ReportExecutionHistory> {
     ...
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId")
    List<ReportExecutionHistory> findByReportId(@Param("reportId") Long reportId);

    /**
     * Find report execution history by report with pagination
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId")
    Page<ReportExecutionHistory> findByReportId(@Param("reportId") Long reportId, Pageable pageable);

    /**
     * Find report execution history by executed by user
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.executedBy = :executedBy")
    List<ReportExecutionHistory> findByExecutedBy(@Param("executedBy") Long executedBy);

    /**
     * Find report execution history by executed by user with pagination
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.executedBy = :executedBy")
    Page<ReportExecutionHistory> findByExecutedBy(@Param("executedBy") Long executedBy, Pageable pageable);

    /**
     * Find report execution history by status
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.status = :status")
    List<ReportExecutionHistory> findByStatus(@Param("status") String status);

    /**
     * Find report execution history by status with pagination
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.status = :status")
    Page<ReportExecutionHistory> findByStatus(@Param("status") String status, Pageable pageable);

    /**
     * Find report execution history by output format
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.outputFormat = :outputFormat")
    List<ReportExecutionHistory> findByOutputFormat(@Param("outputFormat") String outputFormat);

    /**
     * Find report execution history by executed at time range
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.executedAt BETWEEN :startTime AND :endTime")
    List<ReportExecutionHistory> findByExecutedAtBetween(@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);

    /**
     * Find report execution history by executed at time range with pagination
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.executedAt BETWEEN :startTime AND :endTime")
    Page<ReportExecutionHistory> findByExecutedAtBetween(@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime, Pageable pageable);
     ...
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE LOWER(reh.outputFilePath) LIKE LOWER(CONCAT('%', :outputFilePath, '%'))")
    Page<ReportExecutionHistory> findByOutputFilePathContainingIgnoreCase(@Param("outputFilePath") String outputFilePath, Pageable pageable);

    /**
     * Search report execution history by multiple criteria
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE " +
            "(:reportId IS NULL OR reh.report.id = :reportId) AND " +
     ...
     */
    @Query("SELECT COUNT(reh) FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId")
    long countByReportId(@Param("reportId") Long reportId);

    /**
     * Count report execution history by executed by user
     */
    @Query("SELECT COUNT(reh) FROM ReportExecutionHistory reh WHERE reh.executedBy = :executedBy")
    long countByExecutedBy(@Param("executedBy") Long executedBy);

    /**
     * Count report execution history by status
     */
    @Query("SELECT COUNT(reh) FROM ReportExecutionHistory reh WHERE reh.status = :status")
    long countByStatus(@Param("status") String status);

    /**
     * Count report execution history in time range
     */
    @Query("SELECT COUNT(reh) FROM ReportExecutionHistory reh WHERE reh.executedAt BETWEEN :startTime AND :endTime")
     ...
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId AND reh.status = :status")
    List<ReportExecutionHistory> findByReportIdAndStatus(@Param("reportId") Long reportId, @Param("status") String status);

    /**
     * Find report execution history by user and status
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.executedBy = :executedBy AND reh.status = :status")
    List<ReportExecutionHistory> findByExecutedByAndStatus(@Param("executedBy") Long executedBy, @Param("status") String status);
     ...
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId " +
            "AND reh.status = 'COMPLETED' ORDER BY reh.executedAt" )
    ...
    @Query("SELECT " +
            ...
            "SUM(CASE WHEN reh.status = 'FAILED' THEN 1 ELSE 0 END) as failedExecutions, " +
            "AVG(reh.executionTimeMs) as avgExecutionTime " +
            "FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId")
...
//           "GROUP BY HOUR(reh.executedAt) )
//    List<Object[]> getHourlyReportExecutionSummary();
//
    /**
     * Get most executed reports
     */
    @Query("SELECT reh.report.id, COUNT(reh) as executionCount " +
            "FROM ReportExecutionHistory reh WHERE reh.executedAt >= :sinceTime " +
            "GROUP BY reh.report.id ORDER BY executionCount DESC")
     ...
     */
    @Query("SELECT reh.report.id, AVG(reh.executionTimeMs) as avgExecutionTime, " +
            "COUNT(reh) as executionCount " +
            "FROM ReportExecutionHistory reh WHERE reh.status = 'COMPLETED' " +
            "GROUP BY reh.report.id ORDER BY avgExecutionTime DESC")
     ...
     */
    @Query("SELECT reh FROM ReportExecutionHistory reh ORDER BY reh.executedAt DESC")
    ...
    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE reh.report.id = :reportId ORDER BY reh.executedAt DESC")
    Page<ReportExecutionHistory> findByReportIdOrderByExecutedAtDesc(@Param("reportId") Long reportId, Pageable pageable);

    @Query("SELECT reh FROM ReportExecutionHistory reh WHERE " +
            "LOWER(reh.outputFilePath) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
            "LOWER(reh.status) LIKE LOWER(CONCAT('%', :keyword, '%')) OR " +
            "LOWER(reh.outputFormat) LIKE LOWER(CONCAT('%', :keyword, '%'))")
    Page<ReportExecutionHistory> searchExecutionHistories(@Param("keyword") String keyword, Pageable pageable);

    @Modifying
    @Query("DELETE FROM ReportExecutionHistory reh WHERE reh.executedAt < :cutoffDate")
    int deleteByExecutedAtBefore(@Param("cutoffDate") LocalDateTime cutoffDate);
}