...
public interface SyncLogRepository extends JpaRepository<SyncLog, Long>, JpaSpecificationExecutor<SyncLog> {
     ...
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.user.id = :userId")
    List<SyncLog> findByUserId(@Param("userId") Long userId);

    /**
     * Find sync logs by user with pagination
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.user.id = :userId")
    Page<SyncLog> findByUserId(@Param("userId") Long userId, Pageable pageable);

    @Query("SELECT sl FROM SyncLog sl WHERE sl.deviceId = :deviceId")
    List<SyncLog> findByDeviceId(@Param("deviceId") String deviceId);

    @Query("SELECT sl FROM SyncLog sl WHERE sl.syncStatus IN :statuses")
    List<SyncLog> findBySyncStatusIn(@Param("statuses") List<String> statuses);
    ...
    List<SyncLog> findByUserIdAndDeviceId(@Param("userId") Long userId, @Param("deviceId") String deviceId);

    @Query("SELECT sl FROM SyncLog sl WHERE sl.conflictsDetected > :count")
    List<SyncLog> findByConflictsDetectedGreaterThan(@Param("count") int count);

    @Query("SELECT sl FROM SyncLog sl WHERE sl.conflictsResolved < sl.conflictsDetected")
    List<SyncLog> findByConflictsResolvedLessThanConflictsDetected();


    @Query("SELECT sl FROM SyncLog sl WHERE sl.user.id = :userId AND sl.conflictsResolved < sl.conflictsDetected")
    List<SyncLog> findByUserIdAndConflictsResolvedLessThanConflictsDetected(@Param("userId") Long userId);
     ...
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.user.id = :userId AND sl.deviceId = :deviceId")
    Page<SyncLog> findByUserIdAndDeviceId(@Param("userId") Long userId, @Param("deviceId") String deviceId, Pageable pageable);

    /**
     * Find sync logs by device type
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.deviceType = :deviceType")
    List<SyncLog> findByDeviceType(@Param("deviceType") String deviceType);

    /**
     * Find sync logs by device type with pagination
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.deviceType = :deviceType")
    Page<SyncLog> findByDeviceType(@Param("deviceType") String deviceType, Pageable pageable);

    /**
     * Find sync logs by sync status
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.syncStatus = :syncStatus")
    List<SyncLog> findBySyncStatus(@Param("syncStatus") String syncStatus);

    /**
     * Find sync logs by sync status with pagination
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.syncStatus = :syncStatus")
    Page<SyncLog> findBySyncStatus(@Param("syncStatus") String syncStatus, Pageable pageable);

    /**
     * Find sync logs by sync type
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.syncType = :syncType")
    List<SyncLog> findBySyncType(@Param("syncType") String syncType);

    /**
     * Find sync logs by sync direction
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.syncDirection = :syncDirection")
    List<SyncLog> findBySyncDirection(@Param("syncDirection") String syncDirection);

    /**
     * Find sync logs by started at time range
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.startedAt BETWEEN :startTime AND :endTime")
    List<SyncLog> findByStartedAtBetween(@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);

    /**
     * Find sync logs by started at time range with pagination
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.startedAt BETWEEN :startTime AND :endTime")
    Page<SyncLog> findByStartedAtBetween(@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime, Pageable pageable);
     ...
     */
    @Query("SELECT sl FROM SyncLog sl WHERE LOWER(sl.deviceId) LIKE LOWER(CONCAT('%', :deviceId, '%'))")
    Page<SyncLog> findByDeviceIdContainingIgnoreCase(@Param("deviceId") String deviceId, Pageable pageable);
     ...
     */
    @Query("SELECT COUNT(sl) FROM SyncLog sl WHERE sl.syncStatus = :syncStatus")
    long countBySyncStatus(@Param("syncStatus") String syncStatus);

    /**
     * Count sync logs by user
     */
    @Query("SELECT COUNT(sl) FROM SyncLog sl WHERE sl.user.id = :userId")
    long countByUserId(@Param("userId") Long userId);

    /**
     * Count sync logs by user and device
     */
    @Query("SELECT COUNT(sl) FROM SyncLog sl WHERE sl.user.id = :userId AND sl.deviceId = :deviceId")
    long countByUserIdAndDeviceId(@Param("userId") Long userId, @Param("deviceId") String deviceId);

    /**
     * Count sync logs in time range
     */
    @Query("SELECT COUNT(sl) FROM SyncLog sl WHERE sl.startedAt BETWEEN :startTime AND :endTime")
    long countByStartedAtBetween(@Param("startTime") LocalDateTime startTime, @Param("endTime") LocalDateTime endTime);
     ...
     */
    @Query("SELECT sl FROM SyncLog sl WHERE sl.user.id = :userId AND sl.deviceId = :deviceId AND sl.syncStatus = :syncStatus")
    List<SyncLog> findByUserIdAndDeviceIdAndSyncStatus(@Param("userId") Long userId, @Param("deviceId") String deviceId, @Param("syncStatus") String syncStatus);
