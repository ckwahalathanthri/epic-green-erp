...
public class GeneralLedgerController {
    ...
    public ResponseEntity<ApiResponse<Page<GeneralLedger>>> getAccountLedger(
        @PathVariable Long accountId,
        Pageable pageable
    ) {
        Page<GeneralLedger> ledger = accountingService.getAccountLedger(accountId, pageable);
        return ResponseEntity.ok(ApiResponse.success(ledger, "Account ledger retrieved"));
    }
    ...
    public ResponseEntity<ApiResponse<List<GeneralLedger>>> getAccountLedgerForPeriod(
        ...
        return ResponseEntity.ok(ApiResponse.success(ledger, "Period ledger retrieved"));
    }
    
    @GetMapping("/journal-entry/{journalEntryId}")
    @PreAuthorize("hasAnyRole('ADMIN', 'ACCOUNTANT', 'MANAGER')")
    public ResponseEntity<ApiResponse<List<GeneralLedger>>> getLedgerEntriesByJournalEntry(
    ...
    @GetMapping("/search")
    @PreAuthorize("hasAnyRole('ADMIN', 'ACCOUNTANT', 'MANAGER')")
    public ResponseEntity<ApiResponse<Page<GeneralLedger>>> searchLedgerEntries(
